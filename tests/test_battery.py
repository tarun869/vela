...
class TestBatteryDispatch:
    ...
    def test_flat_lmp_no_dispatch(self):
        """With flat LMP and soc_init=0 there is no arbitrage opportunity; total dispatch near zero."""
        optimizer = MILPDispatchOptimizer(horizon=24)
        # Start with empty battery: no stored energy to sell, no incentive to charge at flat prices
        asset = Asset(asset_id="bat-001", capacity_mwh=4.0, power_mw=2.0,
                      soc_init=0.05, soc_min=0.05)  # start at minimum SOC
        ...
        if solution.status in ("Optimal", "Feasible"):
            ...
            # With flat prices and empty battery, minimal or zero net cycling expected
            # Allow for small numeric rounding
            assert abs(total_discharge - total_charge) <= total_charge * 0.15 + 0.5
